...
test.describe('Assessment Selection', () => {
  ...
  test('should display available assessments', async ({ page }) => {
    ...
    const assessments = page.locator(
      '[data-testid="assessment-card"], .assessment-card, a[href*="assessment"]',
    )
  ...
  test('should navigate to PHQ-9 assessment', async ({ page }) => {
    ...
    const phq9Link = page
      .locator('text=PHQ-9, a:has-text("Depression"), button:has-text("PHQ-9")')
      .first()
  ...
  test('should navigate to GAD-7 assessment', async ({ page }) => {
    ...
    const gad7Link = page
      .locator('text=GAD-7, a:has-text("Anxiety"), button:has-text("GAD-7")')
      .first()
...
test.describe('Assessment Flow - PHQ-9', () => {
  ...
  test('should require answering questions before proceeding', async ({ page }) => {
    ...
    const nextButton = page
      .locator('button:has-text("Next"), button:has-text("Submit"), button[type="submit"]')
      .first()
  ...
  test('should complete full assessment', async ({ page }) => {
    ...
    const submitButton = page
      .locator('button:has-text("Submit"), button:has-text("Finish"), button[type="submit"]')
      .first()
  ...
  test('should display results after completion', async ({ page }) => {
    ...
    if ((await submitButton.isVisible()) && !(await submitButton.isDisabled())) {
      ...
      await expect(page.locator('text=/score|severity|result/i').first()).toBeVisible({
        timeout: 5000,
      })
    }
  })
  ...
  test('should allow retaking assessment', async ({ page }) => {
    ...
    const retakeButton = page
      .locator('button:has-text("Retake"), button:has-text("Take Again"), a:has-text("Retake")')
      .first()
...
test.describe('Assessment History', () => {
  test('should show assessment history', async ({ page }) => {
    ...
    const historySection = page
      .locator('text=/history|past|previous/i, [data-testid="assessment-history"]')
      .first()
